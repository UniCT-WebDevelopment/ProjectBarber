    ...
    <script>
        ...
        if(primo == "Lun" || primo == "Mar"){
            ...
            for (var dd in days) {
                ...
                url2 = '{{ route("employee.viewbooking", ":day") }}';
                url2 = url2.replace(':day', days[dd].id);
                url = '{{ route("employee.viewhour", ":day") }}';
                url = url.replace(':day', days[dd].id);

                f = "<div class='question-form'>Vuoi prenotare o cancellare prenotazioni?</div><form action="+url+" method='get'><button type='submit' class='btn-form-day-free'>Hour free</button></form><form action="+url2+" method='get'><button type='submit' class='btn-form-day-booking'>Bookings</button></form>";
